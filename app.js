...
function valuesFromRanges() {
  return Object.fromEntries(Object.entries(ranges).map(([key, input]) => [key, input ? Number(input.value) : 3]));
}

function emitContractChange(values) {
  window.dispatchEvent(new CustomEvent('lab37:contractchange', { detail: { values, scenario: activeScenario } }));
}

function updateReadout() {
  const values = valuesFromRanges();
  ...
  let { state, headline, summary } = base;
  ...
  if (customAdjusted && spread >= 4) {
    ...
    summary = 'Validate intervention frequency, fault context, and time to restore alongside throughput so fast operation also produces fast learning.';
...
function bindControls() {
  ...
  list?.addEventListener('keydown', (event) => {
    ...
    const buttons = [...list.querySelectorAll('.scenario-button')];
  ...
  Object.values(ranges).forEach((input) => input?.addEventListener('input', () => {
    customAdjusted = true;
    updateReadout();
  }));
...
function installPresentationLayer() {
  if (!document.querySelector('link[href="desktop-v2.css"]')) {
    ...
    link.rel = 'stylesheet';
    link.href = 'desktop-v2.css';
    document.head.appendChild(link);
  }
  const stage = document.querySelector('.hero-stage');
  if (stage && !document.querySelector('.stage-cycle')) {
    const panel = document.createElement('div');
    panel.className = 'stage-cycle';
    panel.setAttribute('aria-live', 'polite');
    panel.innerHTML = '<span id="cycle-bowl">Bowl 01</span><strong id="cycle-phase">Filling the bowl</strong><button id="replay-assembly" type="button">Replay cycle</button><i class="cycle-track" aria-hidden="true"><b id="cycle-progress"></b></i>';
...
if (canvas) {
  import('./animation-v2.js')
    .then(({ initBowlAssembly }) => initBowlAssembly({ canvas, reduceMotion }))
    .catch((error) => {
      console.warn('Bowl assembly enhancement unavailable:', error);
